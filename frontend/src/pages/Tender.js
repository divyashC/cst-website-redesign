...
import TenderImg from "../assets/images/news1.png";
import img from "../assets/images/dummyNews.png";


const TenderComponent = ({ imageSrc, views, date, title, content }) => {
  const truncatedContent =
    content && typeof content === 'string' ? content.slice(0, 100) + '...' : '';
  return (
    <div className="flex items-center justify-center w-[640px] mb-[100px] h-auto p-4 cursor-pointer bg-[#D4D4D4] hover:bg-[#D4D4D4] rounded-[20px]" onClick={() => window.location.href='/tender'}>
      <div className="flex-none w-1/3 mr-4">
        <img src={imageSrc} alt="Tender thumbnail" className="w-full h-auto rounded-lg" />
      </div>
  ...
  return (
    <div>
      <div className="relative flex justify-center items-center h-[250px] w-auto">
        <img src={TenderImg} alt="Hero Background" className="w-full -top-[100px] relative -z-30" />
          ...
          <div className="w-full">
            {tenderData.map((tender, index) => {
              if (index % 2 === 0) {
                ...
                return (
                  <div key={tender.id} className="mr-[150px] flex items-center">
